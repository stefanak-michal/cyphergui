...
class Graph extends React.Component<IGraphProps, IGraphState> {
    state: IGraphState = {
        ...
        nodeStyleModal: null,
        edgeStyleModal: null,
        nodeStyles: sessionStorage.getItem('nodeStyles')
            ? JSON.parse(sessionStorage.getItem('nodeStyles'))
            : {},
        edgeStyles: sessionStorage.getItem('edgeStyles')
            ? JSON.parse(sessionStorage.getItem('edgeStyles'))
            : {},
    };
    ...
    componentDidMount() {
        ...
        document
            .getElementById('root')
            .addEventListener('input', this.listenForDarkMode);
    }

    componentWillUnmount() {
        document
            .getElementById('root')
            .removeEventListener('input', this.listenForDarkMode);
    }
        ...
        this.props.rows.forEach(row => {
            for (let key of row.keys) {
                ...
                if (
                    ...
                    !nodes.find(n => n.id === db.strInt(value.identity))
                ) {
                    ...
                    !edges.find(e => e.id === db.strInt(value.identity))
                ) {
                    //prepare data for orb
                    edges.push({
                        id: db.strInt(value.identity),
    ...
    applyNodeStyles = (labels: string[]) => {
        ...
        this.orb.data.getNodes().forEach(node => {
            ...
            if ('shape' in tmpNodeStyles[node.data.element.labels[0]])
                node.style.shape =
                    tmpNodeStyles[node.data.element.labels[0]].shape;
            if ('size' in tmpNodeStyles[node.data.element.labels[0]])
                node.style.size =
                    tmpNodeStyles[node.data.element.labels[0]].size;
            if ('fontSize' in tmpNodeStyles[node.data.element.labels[0]])
                node.style.fontSize =
                    tmpNodeStyles[node.data.element.labels[0]].fontSize;
            if ('label' in tmpNodeStyles[node.data.element.labels[0]]) {
                if (
                    tmpNodeStyles[node.data.element.labels[0]].label in
                    node.data.element.properties
                )
                    node.style.label =
                        node.data.element.properties[
                            tmpNodeStyles[node.data.element.labels[0]].label
                        ];
                else if (
                    tmpNodeStyles[node.data.element.labels[0]].label === '#id'
                )
    ...
    applyEdgeStyles = (types: string[]) => {
        ...
        this.orb.data.getEdges().forEach(edge => {
            ...
            if ('fontSize' in tmpEdgeStyles[edge.data.element.type])
                edge.style.fontSize =
                    tmpEdgeStyles[edge.data.element.type].fontSize;
            if ('label' in tmpEdgeStyles[edge.data.element.type]) {
                if (
                    tmpEdgeStyles[edge.data.element.type].label in
                    edge.data.element.properties
                )
                    edge.style.label =
                        edge.data.element.properties[
                            tmpEdgeStyles[edge.data.element.type].label
                        ];
        ...
        this.setState(
            ...
            () => {
                sessionStorage.setItem(
                    'nodeStyles',
                    JSON.stringify(this.state.nodeStyles)
                );
            }
        );
    ...
    updateEdgeStyle = (
        ...
        this.setState(
            ...
            () => {
                sessionStorage.setItem(
                    'edgeStyles',
                    JSON.stringify(this.state.edgeStyles)
                );
            }
        );
        ...
        return (
            <div className='graph-container is-flex' ref={this.graphContainer}>
                <div
                    className={
                        'graph ' +
                        (this.state.sidebarVisible > 0 ? 'sidebar-visible' : '')
                    }
                ...
                {this.state.sidebarVisible > 0 && (
                    <div
                        className={
                            'sidebar p-3 ' +
                            (this.state.sidebarVisible === 3
                                ? 'animate_out'
                                : '') +
                            (this.state.sidebarVisible === 2
                                ? 'animate_in'
                                : '')
                        }
                        onAnimationEnd={() => {
                    ...
                    >
                        <p className='title is-4 mb-3'>
                            {(this.state.detailHover ||
                                this.state.detail) instanceof _Node
                                ? 'Node'
                                : (this.state.detailHover ||
                                        this.state.detail) instanceof
                                    _Relationship
                                  ? 'Relationship'
                                  : 'Overview'}
                        </p>
                        <div className='content'>
                            <SidebarContent
                                detail={
                                    this.state.detailHover || this.state.detail
                                }
                                labels={this.state.labels}
                                types={this.state.types}
                                labelClick={(label: string) =>
                                    this.setState({ nodeStyleModal: label })
                                }
                                typeClick={(type: string) =>
                                    this.setState({ edgeStyleModal: type })
                                }
                ...
                <div className='buttons'>
                    ...
                    <Button
                        ...
                        onClick={() => this.orb.view.recenter()}
                        color='mr-0'
                        title='Recenter'
                    />
                </div>
                ...
                <div className='brand is-flex is-align-items-center'>
                    ...
                    <a
                        href='https://github.com/memgraph/orb'
                        target='_blank'
                        className='ml-1'
                    >
                ...
                {this.state.nodeStyleModal && (
                    <NodeStyleModal
                        label={this.state.nodeStyleModal}
                        currentSettings={
                            this.state.nodeStyles[this.state.nodeStyleModal]
                        }
                        handleClose={() =>
                            this.setState({ nodeStyleModal: null })
                        }
                ...
                {this.state.edgeStyleModal && (
                    <EdgeStyleModal
                        type={this.state.edgeStyleModal}
                        currentSettings={
                            this.state.edgeStyles[this.state.edgeStyleModal]
                        }
                        handleClose={() =>
                            this.setState({ edgeStyleModal: null })
                        }
                        ...
                        labelFields={
                            this.props.rows.map(record => {
                                for (const key of record.keys) {
                                    const item = record.get(key);
                                    if (
                                        item instanceof _Relationship &&
                                        item.type === this.state.edgeStyleModal
                                    )
                                        return Object.keys(item.properties);
                                }
                            })[0]
