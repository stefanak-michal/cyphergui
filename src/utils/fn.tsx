    ...
    switch (type) {
        ...
        case EPropertyType.Time:
            return [
                ...
                db.strInt((value as _Time).nanosecond),
                db.fromInt((value as _Time).timeZoneOffsetSeconds) / 60 / 60,
            ];
        ...
        case EPropertyType.Map:
            return (
                ...
                (property.value as t_FormValue[])
                    .map(entry => entry.key + ': ' + cypherPrintProperty(entry))
                    .join(', ') +
                '}'
            );
        case EPropertyType.Point:
            return (
                ...
                ', y: ' +
                property.temp[2] +
                (property.temp.length === 4 ? ', z: ' + property.temp[3] : '') +
                '})'
            );
        case EPropertyType.Date:
    ...
    for (const p of properties) {
        if (p.type === EPropertyType.List) {
            ...
            (p.value as t_FormValue[]).forEach(entry => {
                props[p.key].push(entry.value);
            });
        } else if (p.type === EPropertyType.Map) {
            props[p.key] = {};
            (p.value as t_FormValue[]).forEach(entry => {
                props[p.key][entry.key] = entry.value;
            });
        } else {
            props[p.key] = p.value;
        }
    ...
    if (property.constructor.name === 'Object')
        return (
            ...
            Object.keys(property)
                .map(key => key + ': ' + printProperty(property[key]))
                .join(', ') +
            '}'
        );
